#!/usr/bin/env bash
# check-file-size.sh: ファイルサイズに関する警告フック（2機能を同居させる）。
# - PreToolUse(Read): offset/limit指定なしで1000行超または30000字超のファイルを読もうとした場合に、
#   CLAUDE.md「トークン節約の方針」（必要な範囲のみ読む）を思い出させる警告を返す。
# - PostToolUse(Edit|Write): CLAUDE.md/PROJECT.md/SESSION.mdがドキュメント字数上限を超えた場合に警告する。
# いずれも警告のみで操作はブロックしない。
set -euo pipefail
...
# ドキュメント別字数上限（CLAUDE.md「トークン節約の方針」参照）
DOC_LIMIT_CLAUDE=8000
...
check_read_size() {
  ...
  if [ -n "$exceeded" ]; then
    emit_warning "PreToolUse" "ファイル ${FILE_PATH} は ${exceeded} あり、offset/limit指定なしで全文読み込もうとしています。CLAUDE.mdのトークン節約の方針に従い、必要な範囲のみ読むことを検討してください。"
  fi
}
...
check_doc_limit() {
  ...
  if [ "$char_count" -gt "$limit" ]; then
    emit_warning "PostToolUse" "${base} は編集後 ${char_count}字（上限${limit}字）です。CLAUDE.mdのドキュメント字数上限を超えています。内容を削るか置き換えを検討してください。"
  fi
}
