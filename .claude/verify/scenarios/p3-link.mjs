...
export default async function run({ page, check }) {
  ...
  await check('残り時間が分単位で表示される', async () =>
    (await page.textContent('#link-code-remaining')).includes('ふん')
  );
  ...
  await pageB.route(`${ENDPOINT}/sync*`, async (route) => {
    if (route.request().method() === 'GET') {
      await route.fulfill({ json: { events: serverEventsForLearnerA } });
    } else {
  ...
  await check(
    ...
    'コードの きげんが きれています。もういちど はっこうしてください'
  );
