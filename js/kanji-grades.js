// 文部科学省「学年別漢字配当表」（小学校学習指導要領 別表・2020年版）。
// 各学年の配当漢字を文字列で持ち、KANJI_GRADEで「漢字→配当学年(1〜6)」の逆引きを作る。
// 学年別字数は80/160/200/202/193/191（計1026字）。重複なし（`npm run validate:lessons`実行時に
// 暗黙に前提とするのみで、字数・重複そのものは検証しない。出典データは固定のため）。
const GRADE_KANJI = {
...
export const KANJI_GRADE = new Map();
for (const [grade, chars] of Object.entries(GRADE_KANJI)) {
  for (const c of chars) KANJI_GRADE.set(c, Number(grade));
}

// よみレベル。0=ねんちょう（常に全ひらがな）、1〜6=配当学年。ダッシュボードの<select>と
// text-render.jsの表示判定で共有する。
