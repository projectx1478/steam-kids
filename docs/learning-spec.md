...
- 各単元1本目のみ、`intro`直後・`predict`より前に`kind: "tutorial"`（なぞり操作型チュートリアル）
  を置く。文字を読ませない方針のため、手順は文章でなく実物ボタンと同じ見た目の「お手本列」＋
  現在の操作対象だけを光らせる誘導で示す。失敗表示は無く、学習イベント（`run`/`clear`等）も
  記録しない（詳細は`docs/lesson-schema.md`「`tutorial`」。Issue #81）
