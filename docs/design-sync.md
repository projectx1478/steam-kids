...
2台目がリンクコードで同じ学習者IDへ接続した**後**にローカル未設定のまま「初回設定」画面に
入って別の合言葉を作ってしまうと、サーバー側`passHash`（1台目が設定したもの）と食い違い、
2台目では`/guardian/auth`が常に失敗し`/link/issue`が使えなくなる。

さらに、2台目は元々**「初回設定」画面（合言葉ゲート）を通らないとリンクコード入力欄
（ダッシュボードの同期セクション）に到達できない**ため、リンクコードを入力する前に
必ず何らかの合言葉を作らされる構造だった。これが実機確認で見つかった実際の不具合の経路
（#45初回対応はサーバー確認の仕組みだけを入れたが、この経路そのものは塞いでいなかった）。

これを解決するため、「初回設定」画面に**合言葉を決める前にリンクコードで接続できる入口**
（`#gate-setup-link-*`、「他の端末でお使いですか？」）を用意した。

1. 真の初回端末（ローカル未設定・同期未有効）が最初に見るのは常に「初回設定」画面
2. その画面から合言葉を決めずにリンクコードを入力すると`/link/redeem`（無認証）を呼び、
   成功すると学習者IDが引き継がれ同期が有効になる
3. 引き続き画面の出し分けを再判定する。ローカルに合言葉未設定・同期有効になった状態のため
   `GET /guardian/exists`でサーバー側の設定有無を確認する
   - 設定済み（`exists: true`）→「ログイン」画面を出し、入力された合言葉を`/guardian/auth`で
     照合する。成功したらローカルにもハッシュを保存し、以後はその端末でもオフラインで
     解錠できるようにする
   - 未設定（`exists: false`）、またはオフライン等で確認できない → 「初回設定」画面のまま
     （既知の制約。完全に解決するには合言葉自体を暗号的に安全な形で端末間同期する設計が
     要るが、そこまでは行わない）
...
| コード入力 `/link/redeem` | 保護なし（無認証）。ダッシュボードの同期セクション（ローカルゲート通過後）に加え、「初回設定」画面からも合言葉を決める前に呼べる（#45） |
