...
最終更新：2026-09-20（Issue #33の設計方針を承認済み。実装はSonnetへ引き継ぎ）
...
③の現象：端末Bでコード入力→「別の学習者として登録済みです」で同期されない。D1実データとコードで
真因を確定し、**設計方針までIssue #33本文に記載・ユーザー承認済み（2026-09-20）。実装は未着手。**

要点：①`/link/redeem`は失敗してもコードを消費する（ロールバック漏れ）②既に自分のIDで同期済みの端末は
乗り換えできない③`lastPushedTs`/`lastPulledTs`の引き継ぎと`INSERT OR IGNORE`により、②を直すだけでは
履歴統合が完成しない。**2026-09-18の「同期されない」（Issue #22コメント2）も同じ真因で、PR #27は症状へ
の対処だった可能性が高い。**

④（リンクコード入力しやすさ・ダッシュボード読みやすさ）は③が直らないと評価不能のため未確認。

次：Sonnetに切り替えてIssue #33を実装 → ユーザーが`wrangler deploy` → 実機で③④再確認 →
...
Issue #33は設計承認済み。**実装内容・判定可能な数値条件・検証計画はIssue #33本文が正本**。
Opusは実装しない規則のためSonnetで着手する。Worker変更を含むため反映にはユーザーの手動
`wrangler deploy`が必要。完了後、Issue #22の③④を再度実機確認依頼する。
...
- #33 リンクコード引き換えのsecret_conflict失敗とコード消費ロールバック漏れ（設計承認済み・実装待ち）← 今ここ
...
Sonnetに切り替え、Issue #33本文の「実装」に沿って着手する。
