...
最終更新：2026-09-20（#37マージ・クローズ。次は#38着手）
...
**Issue #37（保護者ゲート・クライアント側）はPR #40マージ・クローズ済み（2026-09-20）。**
子ども画面の歯車リンクから`dashboard.html`へ無認証で入れる不備を解消。ユーザーが実機で
パスコード運用を確認し許容と判断済み。

**次はIssue #38（Worker側の保護者トークン）に着手する。** `/link/issue`へのトークン必須化。
#29（X-Worker-Version）と同一の手動`wrangler deploy`にまとめる。
...
Issue #38（同期Workerに保護者トークンを追加）に着手する。実装内容・判定可能な数値条件・検証計画は
本文に揃っている。#38 と #29 は同一の手動`wrangler deploy`にまとめる
（Workerを先にデプロイしないとクライアントから`/guardian/auth`が404になる）。
#31 は #30 完了後に着手する。

保護者ゲートは二層（閲覧解錠＝ローカルPBKDF2照合でオフライン可／#38でサーバー側にHMAC短命
トークンを追加）。ローカル層は devtools・localStorage 編集で迂回可能なことを許容済み。
...
- #38 同期Workerに保護者トークンを追加し `/link/issue` を保護する（要deploy）← 今ここ
...
Issue #38の本文を読み、実装方針を提示して承認を得てから着手する。
