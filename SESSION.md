...
最終更新：2026-09-26（B1(#60)実装完了、PR作成前）
...
**B1（#60）実装完了。PR作成待ち。** ブランチ`claude/b1-donguri`。マージ後、A1〜A5・B1完了
時点での実機確認（子どもの反応）とB2以降・Rの優先度見直しをユーザーと行う（#53）。
拡張プランv2の親トラッキング #53、子Issue #54〜#70（A1〜A5・K1・B1完了）。EN・C〜Eは未起票。

# 完了したタスク（直近のみ・正本はgit/PR履歴とIssue #3）

- **#60実装完了・PR未作成**（2026-09-26）。ブランチ`claude/b1-donguri`。`play.items`（マス座標配列）
  を追加、クリア条件を「ゴール到達」→「ゴール到達 かつ 全item回収」に変更。`engine-grid.js`の
  `simulate()`が`pickups`/`remainingItems`を返すよう拡張、`ui-grid.js`にどんぐりSVGと
  `view.collectItem()`、`validate-lessons.mjs`のBFSにitem回収ビットマスクを追加。新unit
  `donguri`にレッスン2本（`donguri-01-hirou`・`donguri-02-mawarimichi`）を追加し
  `lessons/index.json`を更新。既存シナリオ`lesson-picker.mjs`のレッスン総数期待値を3→5に更新
  （新レッスン追加に伴う正当な変更、他の既存シナリオは無改変で通過）
...
（各子Issueは#53を参照すれば自己完結）。順はA→K→B→R（A・K完了、現在B）。**A1〜A5とB1完了
時点で実機で子どもの反応を確認し、B2以降・Rの優先度を見直す**（B2以降は着手前にユーザーへ確認）。
...
`claude/b1-donguri`ブランチのPRを作成する（未作成の場合）。マージ後、実機確認とB2以降・Rの
優先度見直しをユーザーと行う。
