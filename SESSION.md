...
最終更新：2026-09-20（PR #34 deploy済・実機確認で新規不具合発見。PR #36作成済み・マージ待ち）
...
**PR #34（Issue #33）はマージ・deploy済み。実機確認で①③④は解消・良好だったが、新たに
「直近7日の取り組みの数値が端末によって異なる」不具合を発見。Issue #35を起票しPR #36で修正済み
（未マージ）。**

原因：サーバー側の学習者ID統合（Issue #33）は正しく動作しているが、`pull()`が`since=<lastPulledTs>`
差分取得のため、乗り換えで移管された過去イベント（`ts`が過去のまま）を**乗り換えの当事者でない
端末（コード発行元）が以後取得できない**設計漏れ。ユーザー承認（差分pull廃止・常に全件取得へ）を
得てPR #36で修正、検証コマンド全実行・20シナリオ162チェック全PASS確認済み（詳細はPR #36本文）。

次：PR #36マージ（クライアント側のみの変更のため`wrangler deploy`は不要）→
実機で「直近7日」の数値一致を再確認 → 問題なければIssue #22をクローズして #29 → #30 → #31 の順。
...
- **#35実装**（2026-09-20）。PR #36（未マージ）。全20シナリオ162チェックPASS（既存20/161含む）

# 引き継ぎ事項

PR #36（Issue #35）のマージ待ち。マージ後：
1. GitHub Pagesへ反映後、実機2台で「直近7日の取り組み」の数値が一致することを再確認依頼
   （クライアント側のみの変更のため`wrangler deploy`は不要）
...
- #22 リンクコードUIと複数端末の履歴統合表示（PR #36マージ後の実機再確認待ち）← 今ここ
...
PR #36のマージ状況を確認する。マージ済みなら実機確認（「直近7日」の数値一致）を依頼する。
