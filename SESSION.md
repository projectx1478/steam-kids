...
最終更新：2026-09-17（P1のIssue #8・#9 を起票。次は #8 の実装をSonnetで）
...
Issue #8 → #9 の順に**Sonnetで**実装する（#9 は #8 の `lessons/*.json` が前提）。
Issueに変更内容・検証ルール・完了条件まで記載済みのため、追加の設計判断は不要。

#8 は画面表示を変えない移設のため、既存の検証シナリオ7本が**無修正**で通ることが受け入れ条件。
通らない場合は移設ミスであり、シナリオ側を直さない。
...
- #8 P1-1 レッスンデータをJSONへ外出しし、fetchで読み込む
- #9 P1-2 レッスンJSONのスキーマ検証スクリプトを追加し、スキーマを確定する
...
Issue #8 を読み、`origin/main` 起点のブランチで実装に着手する。
