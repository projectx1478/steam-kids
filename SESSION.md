...
最終更新：2026-09-26（実機確認完了、#80〜#82起票）
...
**実機フィードバック対応。** 実機確認の結果（ゴール誤解・チュートリアル要望・ボリューム不足、
音・演出はOK）を#53にコメント済み。対応Issueを順に1 Issue=1PRで実装（Sonnet）：
**#80 C1 ゴール・指示の明確化 → #81 C2 チュートリアル → #82 C3 ボリューム増**。
設計・完了条件・検証計画は各Issue本文が正。**B2以降・Rは#80〜#82完了後の再実機確認まで保留**。
...
Sonnetで#80（C1）の実装方針（検証計画含む）を提示し、承認後に実装する。
既存シナリオの期待値は変更しない（C2・C3で構造が変わる既存シナリオは凍結fixture＋`page.route`で保護）。
