...
最終更新：2026-09-20（Issue #33実装・PR #34作成。マージとwrangler deploy待ち）
...
**Issue #33を実装しPR #34を作成済み（未マージ）。マージ後、ユーザーの`wrangler deploy`と
実機での③④再確認が必要。**

PR #32の実機確認③（リンクコード引き換えが`secret_conflict`で失敗）の修正。要点：
`handleLinkRedeem`のコード消費UPDATEを検証より前に分離（失敗時の消費を防止）、
`secret_conflict`を廃止し発行元へdevices付け替え（他端末が残る旧IDのイベントは移管しない）、
クライアント側で乗り換え後に`lastPushedTs`/`lastPulledTs`を0リセットし`S.learnerId`も更新。
検証コマンド全実行・20シナリオ161チェック全PASS確認済み（詳細はPR #34本文）。

④（リンクコード入力しやすさ・ダッシュボード読みやすさ）は③が直らないと評価不能のため未確認のまま。

次：PR #34マージ → ユーザーが`wrangler deploy` → 実機で③④再確認 →
...
PR #34（Issue #33実装）のマージ待ち。マージ後：
1. ユーザーが`wrangler deploy`でWorkerへ反映
2. 実機2台でIssue #22の③④を再確認依頼
3. 問題なければIssue #22・#33をクローズし #29 → #30 → #31 の順へ
...
- #33 リンクコード引き換えのsecret_conflict失敗とコード消費ロールバック漏れ（PR #34作成済み・マージ待ち）← 今ここ
- #29 同期Workerとクライアントの版数ズレ検知（X-Worker-Version）← #33マージ・deploy後、次に着手
...
PR #34のマージ状況を確認する。マージ済みならユーザーへ`wrangler deploy`と実機確認（③④）を依頼する。
