...
最終更新：2026-09-25（#57マージ・クローズ、アーカイブ準備）
...
**A5（#58）実装済み、PR #75 レビュー待ち。** マージ後、SESSION.mdを更新してK1（#59）へ進む。
...
- **A5（#58）実装完了・PRレビュー待ち**（2026-09-26）。PR #75。`lessons/index.json`を
  `units`構造へ拡張し、単元マップ（しま）＋レッスン完了スタンプ・単元全クリアの旗を追加
  （`js/ui-picker.js`新規）。入口→レッスンは従来通り1タップ（中間の単元選択画面は挟まない）。
  実装中に発覚した`.claude/verify/run.mjs`のWindows既存バグ（ESM動的importのURL scheme
  エラー）はIssue #76として起票し、本PRで先行修正済み
...
PR #75のマージ・クローズ状況を確認する。マージ済みならSESSION.mdを更新し、K1（#59）へ着手する
（#53と#59を読み、実装方針を提示して承認を得る）。
